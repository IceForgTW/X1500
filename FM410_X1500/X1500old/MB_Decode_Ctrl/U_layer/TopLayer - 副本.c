...
static OS_TMR *LedTimer;	//接收数据定时器	
...
BOOL IsModeToReadCode()
{
	...
	int nGotoSleepDelay = 200;	//读码成功就自动休眠
	int tmp;
...
//						Z_WakeupTimer(200);
			...
			{//距离读出条码在200ms间隔内,则需要适应场景,准备进入侦测状态
				//适应场景
				capture_count++;
				Y_cmosCapture();
				Y_cmosAdjust();
...
/*
	...
	nDevMode
		= DECODE_COLLECT_MODE
*/
static void LedHandlr(void *ptmr, void *parg)
{
    Z_LedOff(LED_GREEN);
}
...
// #endif
		...
		nMstick = Z_Get1msCounter()+ndelay;
        #if 0
		while(Z_Get1msCounter() <= nMstick)
		{
			U_appBKGrndProc();
		}
        ...
        if(LedTimer)
        {
        	if(os_TimerStop(LedTimer, 0, NULL, &err) == 0)
        	{
        		printf("LedTimer: OSTmrStop error\n");

        	}

        	if(os_TimerDelete(LedTimer, &err)== 0)
        	{
        		printf("LedTimer: Del timer error\n");
        	}
            
            LedTimer = NULL;
        }
        LedTimer = os_TimerCreate(ndelay/100, 0, 1, LedHandlr, NULL, "led-off", &err);
        if(err != 0)
        {
            printf("LedTimer: OSTmrCreate  err=%d!\n",err);

        }

        if(os_TimerStart(LedTimer, &err) == 0)
        {
            printf("LedTimer: Start timer err=%d\n",err);
        }
        #endif
	}
// 	else 
...
int U_Set_OutMsg(BYTE *param,BYTE *AOutput,int size)
{
	...
	int j=0;
	int ii=0;
	while(j<size) // ?其他地方会不会出现等于0的情况？
	{
			if(*p == '{')//跳过延时，延时参数一共8个字节，一个字节'{' + 2个字节延时增量 + 4个字节延时次数 + 一个字节'}'
			{
				for (ii=0 ;ii<8 ;ii++)//后面改成以查到'}'结束
				{
					*t++ = *p++;
					j++;
				}
...
int U_DataFormat()
{
	...
	UALLPARAM *psParam;
	int format_Id = -1;
	unsigned int nCodeid,nBigCodeid;
	BYTE Output[1024];
	int Drop_flag = 1;
...
// 	Y_commWrite((unsigned char*)buf, len);
	...
	nCodeid = psParam->pDFormat->format[format_Id].CodeID;//codeID是条码序列号
	if (nCodeid < 32)//1D
	{
		...
		nCodeid = (1<<nCodeid);//解码库给的codeID是 1左移（条码序列号）位得到的
		nBigCodeid = UT_TYPE_1D;//条码大类
	}
	else if (nCodeid < 40)//2D
	{
		nCodeid = (1<<(nCodeid-32));//PDF417还从0开始是为了与使能位置的保持一致
...
//	diag_printf("codeid = %d\r\n",U_appGetCodeType());
//	diag_printf("nBigCodeid = %d\r\n",U_appGetCategory());
	//不符合data Format 的条件
	if ((psParam->pDFormat->format[format_Id].TerID == Y_commGetCurType()  || psParam->pDFormat->format[format_Id].TerID == 999) &&
		((nBigCodeid == U_appGetCategory())&&(nCodeid == U_appGetCodeType()) || psParam->pDFormat->format[format_Id].CodeID ==99) &&
		(psParam->pDFormat->format[format_Id].ComLen >=  U_appGetMsgLen()  || psParam->pDFormat->format[format_Id].ComLen == 9999))
	{
	...
	//	dfparam.nDFSelType = 1;			///1:选择Newland Data Format 方法1;  2:选择Newland Data Format 方法2, 其他方法:待定
		dfparam.nDFSelType = psParam->pDFormat->format[format_Id].FormatType;
	...
	if (nRtn<0)
	{
		if (nRtn == -1)//不符合dataformatting格式的处理
		{
			...
			case 0://按dataformatting格式输出，所有符合格式的条码（若有前后缀）+前后缀输出，所有不符合格式的条码 不输出，且发出或不发出错误提示音//Keep Required and Keep prefix/suffix
				...
				{
					Z_PlayMusic_Ex((char*)SETCODE_ERROR, 10); // 警报声	//提示音
				}
...
// 			if (nRtn == -1000)
// 			{
// 				Y_commSendString("Command error.\r\n");
// 			}
// 			else if (nRtn == -1001)
// 			{
// 				Y_commSendString("String add error.\r\n");
// 			} 
// 			else if (nRtn == -1002)
// 			{
// 				Y_commSendString("Delete string by position error.\r\n ");
// 			} 
// 			else if (nRtn = -1003)
// 			{
// 				Y_commSendString("Delete string error.\r\n");
// 			} 
// 			else if (nRtn == -1004)
// 			{
// 				Y_commSendString("Exacting the fixed position sting error.\r\n");
// 			} 
// 			else if (nRtn == -1006)
// 			{
// 				Y_commSendString("Illicit command.\r\n");
// 			} 
// 			else
// 			{
// 				Y_commSendString("Other error.\r\n");
// 			}
			Y_commSendString("Dataformatting command error.\r\n");
			Z_PlayMusic_Ex((char*)SETCODE_ERROR, 1);
...
// 	else
// 	{
// 		if(!U_Set_OutMsg(Output,AOutput,dfparam.nDFOutputLen))
// 			return -2;
		U_Set_OutMsg(Output,AOutput,dfparam.nDFOutputLen);
