...
typedef struct os_event {
	...
	INT32U   OSPendRA;  //return addr on pending
	INT32U   OSPendSP;  //sp value on pending
#endif
...
extern POSTaskBSPHook g_pBSP;
#define BSPHook(x) {if((g_pBSP)&&(g_pBSP->x)) g_pBSP->x();} 
#define BSPHookArg(x,arg) {if((g_pBSP)&&(g_pBSP->x)) g_pBSP->x(arg);}
#define BSPHookRet(x) ({((g_pBSP)&&(g_pBSP->x))? g_pBSP->x():0;}) 
...
// add by xliu, 20090211
#if OS_TASK_CREATE_EN > 0
int           OSTaskInfo            (void           (*add_info)(int prio, int stat));
#endif
