...
static ST_datetime st_EU_sample={2013,3,0,5,12,0,0};
...
void rtc_get_cur_urtime(ST_datetime *st_sd)
{
	...
	__uint64_t tempam_cur=rtc_read_reg(RTC_RTCSR);
...
//st_time_atom_base
	...
	st_time_atom=tempam_cur;
	rtc_write_reg(RTC_RTCSR, tempam_cur-st_time_atom_base);
	spin_unlock_irqrestore(flags);
}
//=====================================================================
...
//st_time_atom_base
	...
	rtc_write_reg(RTC_RTCSR, u64_time-st_time_atom_base);
	spin_unlock_irqrestore(flags);
}
...
void rtc_alarm_stop()
{
	...
	rtc_clr_reg(RTC_RTCCR, RTC_RTCCR_AF | RTC_RTCCR_AIE);
	rtc_clr_reg(RTC_HWCR, RTC_HWCR_EALM);
...
{
	unsigned int tmpx = rtc_read_reg(RTC_HSPR), flag;
	...
	flag = rtc_read_reg(RTC_RTCGR) & RTC_RTCGR_NC1HZ_MASK;
	if((tmpx != COLD_BOOT_SIG) || (flag != RTCGR_DIV_1HZ))
	{
		printf("tmpx=0x%x, rtcgr=0x%x\n", tmpx, rtc_read_reg(RTC_RTCGR));
		u32_RTC_time_l=0;
		u32_RTC_time_h=0;
 		atom_time_to_ymd(st_time_atom,&st_EU_time);  //get base time info

		rtc_write_reg(RTC_RTCGR, RTCGR_DIV_1HZ);
		rtc_write_reg(RTC_HSPR, COLD_BOOT_SIG);
		rtc_write_reg(RTC_RTCSR, 0x00000000);

		rtc_set_rtc_abs(st_time_atom_base);
		rtc_set_rtc_datetime(&st_EU_sample);

		rtc_set_reg(RTC_RTCCR, RTC_RTCCR_RTCE);
	}
	else
	{
...
/*
//	moidfy by jliu@ingenic.cn 
//	when the bit RTCCR_AE and RTCCR_AIE need to set at the same time 
//	the first bit will be flushed 
*/
int SetRtcAlmTime(unsigned int alarm_time)
{
	...
	unsigned int rtccr = rtc_read_reg(RTC_RTCCR);
	unsigned int tick = alarm_time + rtc_read_reg(RTC_RTCSR);

	printf("rtc alarm will be %d second late!\n", alarm_time);
	rtc_write_reg(RTC_RTCSAR, tick);
	
	if (!(rtccr & RTC_RTCCR_AE)) {
		rtc_write_reg(RTC_RTCCR, rtccr | RTC_RTCCR_AE);
	}
	
	rtccr = rtc_read_reg(RTC_RTCCR);
	if ( !(rtccr & RTC_RTCCR_AIE) ) { /* Enable alarm irq */
		rtc_write_reg(RTC_RTCCR, rtccr | RTC_RTCCR_AIE);
	}
	return 0;
}
...
{ 
	return (rtc_read_reg(RTC_RTCSAR));
}
