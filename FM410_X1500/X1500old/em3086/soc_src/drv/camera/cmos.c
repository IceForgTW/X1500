...
#else
	__gpio_ack_irq((GPIO_GROUP_A + 10));
	if (!__gpio_get_pin(GPIO_GROUP_A + 10))
	{
		if (_light_ctrl)
			_light_ctrl(FALSE);
		//printf("fall edge interrupt\r\n");
		__gpio_as_irq_rise_edge(GPIO_GROUP_A + 10);
	}
	else
	{
		if (_light_ctrl)
			_light_ctrl(TRUE);
		//printf("rise edge interrupt\r\n");
		__gpio_as_irq_fall_edge(GPIO_GROUP_A + 10);
	}
#endif
...
#else
	request_irq(EIRQ_GPIO_BASE + (GPIO_GROUP_A + 10), vsync_handle, 0);
	disable_irq(EIRQ_GPIO_BASE + (GPIO_GROUP_A + 10));
#endif
}
...
void illumination_interrupt_reset(void)
{
	...
	else if(cmosInfo.nShutterType == CMOS_SHUTTER_TYPE_ROLLING)
	{
		...
		__gpio_ack_irq(GPIO_GROUP_A + 10);
		__gpio_as_irq_rise_edge(GPIO_GROUP_A + 10);
		enable_irq(EIRQ_GPIO_BASE + (GPIO_GROUP_A + 10));
#endif
