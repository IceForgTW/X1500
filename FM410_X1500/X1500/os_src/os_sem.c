...
{
#if OS_EVENT_PENDING_WATCH
	unsigned int reg_ra, reg_sp;
	__asm__ __volatile__(
			"sw $31,0x00(%0)\n\t"
			"sw $29,0x00(%1)\n\t"
            :                  
			: "r" (&reg_ra), "r" (&reg_sp));
#endif //OS_EVENT_PENDING_WATCH
    ...
    OS_CPU_SR  cpu_sr = 0;
		...
		if(timeout > 0)
		{
			if(g_pBSP && g_pBSP->Delay)
			{
				g_pBSP->Delay(timeout);
			}
		}	
...
#if OS_EVENT_PENDING_WATCH
	pevent->OSPendRA = reg_ra;
	pevent->OSPendSP = reg_sp;
#endif //OS_EVENT_PENDING_WATCH
...
typedef struct OS_EventPendSnapshot
{
	INT8U TaskPrio;
	INT16U TimeOut;
	OS_EVENT * Event;
	INT32U OSPendSP;
	INT32U OSPendRA;
} OS_EventPendSnapshot;

#define MAX_EVENT_SNAPSHOT 30

OS_EventPendSnapshot EventPendSnapshotList[MAX_EVENT_SNAPSHOT];

int OSWatchPendingTask(void (*WatchTaskCallback)(INT8U TaskPrio, 
	OS_EVENT * CurEvent, INT32U TimeOut, INT32U sp, INT32U ra))
{
#if OS_CRITICAL_METHOD == 3                           /* Allocate storage for CPU status register      */
	OS_CPU_SR  cpu_sr = 0;
#endif
	OS_TCB    *ptcb;
	OS_EventPendSnapshot* Snapshot;
	OS_EventPendSnapshot* EndSnapshot;
	...
	if (OSRunning == OS_TRUE) {
		...
		Snapshot = &EventPendSnapshotList[0];
		EndSnapshot = &EventPendSnapshotList[MAX_EVENT_SNAPSHOT]; 
		
		//生成快照
		//采用先生成快照再输出，而不直接输出的方法是因为输出中会有打印，导致中断屏蔽时间过长，会丢中断。
		while ((ptcb) && (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) && (Snapshot < EndSnapshot)) {
            if ((ptcb->OSTCBStat & OS_STAT_SEM) != OS_STAT_RDY) {
				OS_EVENT * event = ptcb->OSTCBEventPtr;
				if ((event != NULL) && (WatchTaskCallback != NULL)) {

					Snapshot->TaskPrio = ptcb->OSTCBPrio;
					Snapshot->TimeOut = ptcb->OSTCBDly;
					Snapshot->Event = event;
					Snapshot->OSPendSP = event->OSPendSP ;
					Snapshot->OSPendRA = event->OSPendRA;
		...
		//设置结束标志
		if (Snapshot < EndSnapshot)
		{
			Snapshot->TaskPrio = 0; //TaskPrio值为0表示结束
		}

		OS_EXIT_CRITICAL();
		
		//循环输出快照。
		for(Snapshot = &EventPendSnapshotList[0]; Snapshot<EndSnapshot; Snapshot++)
		{
			if (Snapshot->TaskPrio == 0) break;

			WatchTaskCallback(Snapshot->TaskPrio, Snapshot->Event, 
				Snapshot->TimeOut, Snapshot->OSPendSP, Snapshot->OSPendRA);
		}
	}
        return 0;
}
