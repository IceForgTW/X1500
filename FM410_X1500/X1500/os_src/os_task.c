...
unsigned char OSGetCurrentTaskPrio()
{
	unsigned char Prio = OS_LOWEST_PRIO + 1;
...
int OSTaskInfo(void (*add_info)(int prio, int stat))
{
	...
	for(i=0; i<OS_LOWEST_PRIO; i++){
		if(OSTCBPrioTbl[i] != 0){
			if(0 != add_info){
			    ptcb = OSTCBPrioTbl[i];
			    add_info(ptcb->OSTCBPrio, ptcb->OSTCBStat);
			}
			n ++;
		}
	}
...
INT8U OSTaskGetCurPrio(void)
{
	  OS_CPU_SR    cpu_sr = 0;  
   OS_ENTER_CRITICAL();
	 int tmpPrio = OSTCBCur->OSTCBPrio;//OSTCBCur->OSTCBPrio;
	 OS_EXIT_CRITICAL();
	 return tmpPrio;//
}
