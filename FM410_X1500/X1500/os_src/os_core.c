...
void  OSTimeTick (void)
{
    ...
    BSPHook(TimeTick);                                      /* Call user definable hook                     */
#endif
...
int  OSTimeTicks (unsigned short ticks)
{
    OS_TCB    *ptcb;
		int nextticks = 0xffff;
    ...
    if (OSRunning == OS_TRUE) {
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            //OS_ENTER_CRITICAL();            
          	if (ptcb->OSTCBDly != 0)
          	{
          		if(ptcb->OSTCBDly > ticks)
              {	
              	ptcb->OSTCBDly -= ticks;
              	if((ptcb->OSTCBDly < nextticks) && (ptcb->OSTCBDly > 0)) nextticks = ptcb->OSTCBDly;
