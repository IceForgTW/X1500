...
typedef  struct  os_tmr {
    ...
    unsigned char            OSTmrName[OS_TMR_CFG_NAME_SIZE]; /* Name to give the timer                                        */
#endif
    unsigned char            OSTmrOpt;                        /* Options (see OS_TMR_OPT_???)                                  */
...
unsigned char       os_InitTCB                  (unsigned char           prio, 
                                                   ...
                                                   void                   *pext, 
                                                   unsigned short          opt);
OS_TMR      *os_TimerCreate            (unsigned int           dly, 
                                     unsigned int           period, 
                                     unsigned char            opt, 
...
unsigned char      os_TimerDelete               (OS_TMR          *ptmr,
                                     unsigned char           *perr);

unsigned char        os_TimerNameGet           (OS_TMR          *ptmr, 
                                     unsigned char           *pdest, 
                                     unsigned char           *perr);
unsigned int       os_TimerGetRemainTime         (OS_TMR          *ptmr, 
                                     unsigned char           *perr);

unsigned char        os_TimerGetStatus          (OS_TMR          *ptmr, 
                                     unsigned char           *perr);

unsigned char      os_TimerStart             (OS_TMR          *ptmr,
                                     unsigned char           *perr);

unsigned char      os_TimerStop              (OS_TMR          *ptmr, 
                                     unsigned char            opt, 
                                     void            *callback_arg, 
                                     unsigned char           *perr);

unsigned char        os_TimerSignal            (void);

unsigned char os_GetCurrentTaskPrio();

//取栈的深度，即栈的使用量，单位byte
unsigned int os_GetTaskStackDepth();

int OSWatchPendingTask  (void (*WatchTaskCallback)(unsigned char TaskPrio, 	OS_EVENT * CurEvent, unsigned int TimeOut, unsigned int sp, unsigned int ra));
