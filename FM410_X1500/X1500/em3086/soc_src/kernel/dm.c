...
int dm_preconvert(char *name,PPLL_OPT pllopt)
{
	...
	while(dm_end)
	{
		if(dm_end->name && (strcmp((const char *)name,(const char *)dm_end->name) == 0) && (dm_end->preconvert))
...
int dm_convert(char *name,int val)
{
	...
	while(dm_end)
	{
		if(dm_end->name && (strcmp((const char *)name,(const char *)dm_end->name) == 0) && (dm_end->convert))
