/*
 * linux/include/asm-mips/mach-jz4775/jz4775ssi.h
 *
 * JZ4775 SSI register definition.
 *
 * Copyright (C) 2010 Ingenic Semiconductor Co., Ltd.
 */

#ifndef __JZ4775SSI_H__
#define __JZ4775SSI_H__
 ...
 *************************************************************************/
/* n = 0, 1 (SSI0, SSI1) */
#define	SSI_DR		( SSI0_BASE + 0x000 )
#define	SSI_CR0		( SSI0_BASE + 0x004 )
#define	SSI_CR1		( SSI0_BASE + 0x008 )
#define	SSI_SR		( SSI0_BASE + 0x00C )
#define	SSI_ITR		( SSI0_BASE + 0x010 )
#define	SSI_ICR		( SSI0_BASE + 0x014 )
#define	SSI_GR		( SSI0_BASE + 0x018 )
#define SSI_RCNT	( SSI0_BASE + 0x018 )
...
#define REG_SSI_RCNT		REG32(SSI_RCNT)
...
#define RENDIAN_MSB_MSB			(0 << SSI_CR0_RENDIAN_BIT)
#define RENDIAN_MSB_LSB			(1 << SSI_CR0_RENDIAN_BIT)
#define RENDIAN_LSB_LSB			(2 << SSI_CR0_RENDIAN_BIT)
#define RENDIAN_LSB_MSB			(3 << SSI_CR0_RENDIAN_BIT)
#define SSI_CR0_TENDIAN_BIT		18
#define SSI_CR0_TENDIAN_MASK		(0x3 << SSI_CR0_TENDIAN_BIT)
#define TENDIAN_MSB_MSB			(0 << SSI_CR0_TENDIAN_BIT)
#define TENDIAN_MSB_LSB			(1 << SSI_CR0_TENDIAN_BIT)
#define TENDIAN_LSB_LSB			(2 << SSI_CR0_TENDIAN_BIT)
#define TENDIAN_LSB_MSB			(3 << SSI_CR0_TENDIAN_BIT)
...
#define __ssi_select_ce2() 	(REG_SSI_CR0 |= SSI_CR0_FSEL)

#define __ssi_underrun_auto_clear() 		\
do { 						\
	REG_SSI_CR0 |= SSI_CR0_EACLRUN; 	\
} while (0)

#define __ssi_underrun_clear_manually() 	\
do { 						\
	REG_SSI_CR0 &= ~SSI_CR0_EACLRUN; 	\
} while (0)
...
#define __ssi_use_busy_wait_mode() 	__ssi_wait_transmit()
#define __ssi_unset_busy_wait_mode() 	__ssi_finish_transmit()
...
#define __ssi_set_frame_length(m)					\
	REG_SSI_CR1 = (REG_SSI_CR1 & ~SSI_CR1_FLEN_MASK) | (((m) - 2) << 3) 

/* m = 1 - 16 */
#define __ssi_set_microwire_command_length(m)				\
	( REG_SSI_CR1 = ((REG_SSI_CR1 & ~SSI_CR1_MCOM_MASK) | SSI_CR1_MCOM_##m##BIT) )

/* Set the clock phase for SPI */
#define __ssi_set_spi_clock_phase(m)					\
	( REG_SSI_CR1 = ((REG_SSI_CR1 & ~SSI_CR1_PHA) | (((m)&0x1)<< 1)))

/* Set the clock polarity for SPI */
#define __ssi_set_spi_clock_polarity(p)				\
	( REG_SSI_CR1 = ((REG_SSI_CR1 & ~SSI_CR1_POL) | ((p)&0x1)) )

/* SSI tx trigger, m = i x 8 */
#define __ssi_set_tx_trigger(m)				\
	do {							\
		REG_SSI_CR1 &= ~SSI_CR1_TTRG_MASK;		\
		REG_SSI_CR1 |= ((m)/8)<<SSI_CR1_TTRG_BIT;	\
	} while (0)

/* SSI rx trigger, m = i x 8 */
#define __ssi_set_rx_trigger(m)				\
	do {							\
		REG_SSI_CR1 &= ~SSI_CR1_RTRG_MASK;		\
		REG_SSI_CR1 |= ((m)/8)<<SSI_CR1_RTRG_BIT;	\
	} while (0)
...
#define __ssi_clear_errors()		( REG_SSI_SR &= ~(SSI_SR_UNDR | SSI_SR_OVER) )

#define __ssi_set_clk( dev_clk, ssi_clk)			\
	( REG_SSI_GR = (dev_clk) / (2*(ssi_clk)) - 1 )
#define __ssi_set_grdiv(v)	(REG_SSI_GR = v)
#define __ssi_get_grdiv()	REG_SSI_GR
